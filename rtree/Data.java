...
* Data
*/
public class Data implements Comparable, Streamable, Serializable
{
  static final int sizeof_dimension = 4;
  static final int sizeof_float     = 4;

  public  double[] data      = null;   // Vector
  public  double   distanz   = 0;      // anything

  public int dimension = 0;      // array size of data[]
  public int id = 0;

  private ConcurrentLinkedQueue<Location> points;
...
//--------------------------------------------------------------------------
  ...
  */
  public Data()
  {
    ...
    points = new ConcurrentLinkedQueue<Location>();
  }
  /**
...
//--------------------------------------------------------------------------
  ...
  */
  protected void setDimension(int _dimension)
  {
    ...
    this.data = new double[_dimension*2];
  }
...
//--------------------------------------------------------------------------
  ...
  */
  public double[] get_mbr()
  {
    ...
    double[] f = new double[2 * this.dimension];
...
//    String answer = this.getClass().getName();
    String answer = "(distanz="+this.distanz+","+this.dimension+":[";
    answer += Arrays.toString(points.toArray());
//    if (this.dimension > 0)
...
//--------------------------------------------------------------------------
  ...
  */
  ...
  public synchronized List<Location> getPoints(){
//      lock.tryLock();
      List<Location> newPoints = new LinkedList<Location>();
//      try{      

          long currentTime = System.currentTimeMillis();
          for(Location p : points){
    //          if((currentTime - p.getTimestamp()) >= Constants.MAX_POINT_LIFETIME){
    //              points.remove(p);
    //          }
    //          else{
                  newPoints.add(p);
