...
public final class RTree implements Serializable 
{
    ...
    */
    ...
    public void insertPoint(Location p){
        SortedLinList list = new SortedLinList();
        
        point_query(p, list);
        
       if(list.myCount == 0){
           Data d = createDataFromPoint(p);
           insert(d);
       }else{
           for(int i = 0; i< list.myCount; i++){
    ...
    private Data createDataFromPoint(Location p){
        
        Data d = new Data(p.dimension);

        d.data[0] = p.data[0] - minDist;
        d.data[1] = p.data[0] + minDist;
        d.data[2] = p.data[1] - minDist;
        d.data[3] = p.data[1] + minDist;
    ...
    */
    void NearestNeighborQuery(Location QueryPoint, Location Nearest)
    {
    ...
    * to parameter point. --> Propagate to the root node.
    */
    void NearestNeighborQuery(Location QueryPoint, SortedLinList res)
    {
    ...
    * to parameter point. --> Propagate to the root node.
    */
    void k_NearestNeighborQuery(Location QueryPoint, int k, SortedLinList NeighborList)
    {
        int i,l,n;
        float nearest_distanz;
        Data p;
    ...
    */
    ...
    public Set<Location> getPointsInRange(Location center, float radius){
        SortedLinList res = new SortedLinList();
        
        rangeQuery(center, radius, res);       
        
//        System.out.println("Resultado inicial de range query para [" +center +", "+radius+ "] " + res);

        Set<Location> points = new HashSet<Location>();
        for(int i = 0; i< res.myCount; i++){
            Data d = (Data) res.get(i);
            List<Location> ps = d.getPoints();
            
            for(Location p : ps){
                float dist = Constants.DIST(center, p);
//                System.out.println(center + " " + p + " " + dist + " " + radius);
                if(dist < radius){
                    points.add(p);
                }
            }
        }        
//        System.out.println("Resultado final de range query para [" +center +", "+radius+ "] : " + Arrays.toString(points.toArray()));
//        System.out.println("---------------------------------------------");
        return points;
    }
    ...
    */
    ...
    void range_nnQuery(double mbr[], SortedLinList res, Location Nearest)
    {
        Location center;
    ...
    void overlapping(double p[], int nodes_t[])
    {
        load_root();
